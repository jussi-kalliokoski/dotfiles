...
function! OpenAndCreatePaths(filename)
    silent exe "!mkdir -p \"$(dirname \"" . a:filename . "\")\""
    exe "tabnew " . a:filename
endfunction

command! -nargs=1 -complete=file TabCreate call OpenAndCreatePaths(<f-args>)

function! Rename(filename)
    silent exe "!mkdir -p \"$(dirname \"" . a:filename . "\")\""
    let l:curfile = expand("%:p")
    let v:errmsg = ""
    silent! exe "saveas " . a:filename
    if v:errmsg =~# '^$\|^E329'
    ...
    " For some reason everything is broken after renaming
    redraw!
endfunction
